    ...
    if keyword == "map":
        ...
        for k, token in enumerate(tokens[index + 1 : index2], index + 1):
            ...
            if token.content.replace("*", "") == "Pass arguments":
                start = True
            if start and token.content.islower() and token.type == "inline":
        ...
        if (
            ...
            if (
                ...
                schemas[filetype]["properties"]["set"]["properties"][keyword][
                    ...
                    .rpartition("Possible values are ")[2]
            ...
            for line in token.content.splitlines():
                if line.find("Value type: ") != -1:
                    _type = line.strip().partition("Value type: ")[2].lower()
                    schemas[filetype]["properties"]["set"]["properties"][
                        keyword
                    ]["type"] = {
                        "int": "integer",
...
schemas[filetype]["properties"]["set"]["properties"]["selection-clipboard"][
    "enum"
] = ["clipboard", "primary"]
